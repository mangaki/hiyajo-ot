...
class Embedding:
    ...
    def from_filename(cls, embedding_filename, ignore_posters=False):
        ...
        if not ignore_posters:
            embedding.read_posters_paths_from_filename(posters_filenames, data_directory)
...
def generate_mapping(work_directory, ignore_posters: bool = False):
    ...
    for filename in files_list:
        if filename.startswith('embeddings-'):
            try:
                embedding = Embedding.from_filename(os.path.join(work_directory, filename), ignore_posters=ignore_posters)
