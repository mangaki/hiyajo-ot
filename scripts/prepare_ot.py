        ...
        with open(path, "r") as f:
            ...
            for field in mandatory_fields:
                if field not in local_rating_values:
                    raise ValueError(
                        "Incorrect local rating values file, missing field: {} !".format(
                            field
                        )
                    )
...
def warn_for_sequential_ids_mismatches(embeddings):
    ...
    if n_samples > 0:
        logger.warning(
            "There are at least {} missing embeddings, the results might be wrong.".format(
                n_samples
            )
        )
...
def warn_for_size_mismatches(embeddings):
    for embedding in embeddings:
        ...
        if n_samples != n_ids:
            logger.warning(
                "There are {} samples in {} and {} work IDs fetched from the path file".format(
                    n_samples, embedding.npy_filename, n_ids
                )
            )
...
@instrument
def filter_ratings_based_on_cost_matrix_encoder(
    X, y, encoder, sanity_check: bool = False
):
...
def filter_ratings_based_on_cost_matrix(C, X, y, encoder, sanity_check: bool = False):
    X, y, nb_users, nb_works = filter_ratings_based_on_cost_matrix_encoder(
        X, y, encoder, sanity_check
    )
    C = C[:nb_works]

    logger.info("Cost matrix (final) shape: {}".format(C.shape))
    logger.info(
        "Ratings has {} users, {} works, X has shape {}, y has shape {}".format(
            nb_users, nb_works, X.shape, y.shape
        )
    )
...
def create_cost_matrix(X, y, embeddings, sanity_check: bool = False):
    ...
    C, X, y, nb_users, nb_works = filter_ratings_based_on_cost_matrix(
        C, X, y, encoder, sanity_check
    )
...
def compute_user_distribution(user_id, items_matrix, method="divide", epsilon=1e-2):
    ...
    if method == "softmax":
        ...
        assert callable(
            method
        ), "The method to compute user distribution must be a callable if not softmax or divide"
        A = method(A)
...
def compute_user_distributions(nb_users, items_matrix, method="divide", epsilon=1e-2):
    local_cud = lambda user_id: compute_user_distribution(
        user_id, items_matrix, method, epsilon
    )
...
def main():
    ...
    # Mandatory
    parser.add_argument(
        "data_path", type=str, help="Path to the data directory (ratings, embeddings)"
    )
    parser.add_argument(
        "--output",
        default="ot.npy",
        help="Output file consisting of (knn.M original matrix, cost matrix, user distributions matrix, item encoder), default: ot.npy",
    )
    # Utilities
    ...
    parser.add_argument(
        "--sanity-check",
        action="store_true",
        help="Run the sanity check (assertions, it is *SLOWER*)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Limit the amount of users loaded from ratings (default: 100, use 0 for +∞)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="divide",
        help="Method to use to compute the user distribution from KNN matrix (default: divide by sum)",
    )
    parser.add_argument(
        "--epsilon",
        type=float,
        default=1e-2,
        help="Epsilon to perturb an user distribution (lower makes the result more exact, default: 1e-2)",
    )
    # KNN parameters
    parser.add_argument(
        "--knn-neighbors",
        type=int,
        default=20,
        help="Number of neighbors for MangakiKNN instance (default: 20)",
    )
    parser.add_argument(
        "--knn-rated-by-neighbors-at-least",
        type=int,
        default=3,
        help="Rated by neighbors at least parameter for MangakiKNN instance (default: 3)",
    )
    parser.add_argument(
        "--knn-weighted-neighbors",
        action="store_true",
        help="Enable weighted neighbors for MangakiKNN instance",
    )
    ...
    if os.path.isfile(args.output) and not os.access(args.output, os.W_OK):
        raise ValueError(
            "Output file cannot be accessed in write mode, aborting the preparation"
        )
    ...
    knn = MangakiKNN(
        nb_neighbors=args.knn_neighbors,
    ...
    user_distributions = compute_user_distributions(
        nb_users, knn.M, args.method, args.epsilon
    )
    ...
    if args.sanity_check:
        ...
        logger.info(
            "User distributions[{}]: {}".format(
                user_distributions.shape, user_distributions
            )
        )
    try:
        # write (knn.M, C, user_distributions, encoder) into output
        with open(args.output, "wb") as f:
            raw_repr = pickle.dumps(
                (knn.M, C, user_distributions, encoder), pickle.HIGHEST_PROTOCOL
            )
