...
@instrument
def compute_all_wasserstein_from(
    src,
    user_distributions,
    M,
    sinkhorn_reg,
    ...
    try:
        if enable_non_convex_lasso:
            if max_iterations >= 1000:
                logger.warning(
                    "When you enable non-convex LASSO for Sinkhorn algorithm, the default max iterations (of the Sinkhorn solver) is around ~ 200, are you sure to proceed with {} max iterations?".format(
                        max_iterations
                    )
                )
...
def main():
    parser = argparse.ArgumentParser(
        prog="gpu_sinkhorn", description="Run Sinkhorn on GPU over prepared OT data"
    )
    parser.add_argument(
        "input_ot_workload",
        help="The input NumPy arrays pickled file in the format specified in README.md",
    )
    parser.add_argument(
        "output_wasserstein_matrix",
        help="The output NumPy matrix which are the pairwise Wasserstein distances",
    )
    parser.add_argument("--chrono", action="store_true", help="Enable the chronometer")
    parser.add_argument(
        "--use-cpu",
        action="store_true",
        help="Disable GPU usage (useful for debugging as this is GPU/CPU agnostic)",
    )
    parser.add_argument(
        "--enable-pot-logging",
        action="store_true",
        help="Pass verbose=True to POT functions",
    )
    parser.add_argument(
        "--non-convex-lasso",
        action="store_true",
        help="Enable non-convex LASSO regularization (experimental)",
    )
    parser.add_argument(
        "--metric",
        default="sqeuclidean",
        help="Metric to pass to scipy.spatial.distance.pdist for pairwise computation (default: sqeuclidean)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        default=1000,
        help="Max number of iterations for Sinkhorn",
    )
    parser.add_argument(
        "-e",
        ...
        default=0.1,
        type=float,
        help="Nonconvex LASSO regularization",
    )
    ...
    if not os.path.isfile(args.input_ot_workload) and not os.access(
        args.input_ot_workload, os.R_OK
    ):
        raise ValueError(
            "{} is not reachable (permissions or does not exist)".format(
                args.input_ot_workload
            )
        )

    if os.path.isfile(args.output_wasserstein_matrix) and not os.access(
        args.output_wasserstein_matrix, os.W_OK
    ):
        raise ValueError(
            "{} is not writeable (permissions)".format(args.output_wasserstein_matrix)
        )
    ...
    ws_matrix = compute_all_wasserstein(
        user_distributions,
        M,
        args.sinkhorn_regularization,
        args.lasso_regularization,
        args.non_convex_lasso,
        args.sinkhorn_max_iterations,
        args.enable_pot_logging,
        args.use_cpu,
    )
